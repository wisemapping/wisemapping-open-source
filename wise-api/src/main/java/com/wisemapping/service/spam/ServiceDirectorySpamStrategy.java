...
public class ServiceDirectorySpamStrategy implements SpamDetectionStrategy {
    ...
    private static final List<Pattern> CONTACT_PATTERNS = Arrays.asList(
            ...
            Pattern.compile("\\+?[1-9]\\d{1,3}[-.\\s]?\\d{3,4}[-.\\s]?\\d{3,4}[-.\\s]?\\d{3,4}",
                    Pattern.CASE_INSENSITIVE), // International format with spaces/dashes (e.g., +61 1300 650 773)
            ...
            Pattern.compile("[0-9]{3}[-.\\s]?[0-9]{3}[-.\\s]?[0-9]{4}", Pattern.CASE_INSENSITIVE), // US format without
                                                                                                   // parentheses
            Pattern.compile("0\\d{2,4}[-.\\s]?\\d{3,4}[-.\\s]?\\d{3,4}", Pattern.CASE_INSENSITIVE), // UK format (starts
                                                                                                    // with 0, e.g.,
                                                                                                    // 01273 782 734,
                                                                                                    // 01754 768120)
            Pattern.compile("\\+44[-.\\s]?\\d{2,4}[-.\\s]?\\d{3,4}[-.\\s]?\\d{3,4}", Pattern.CASE_INSENSITIVE), // UK
                                                                                                                // international
                                                                                                                // format
                                                                                                                // (+44)
            // Swiss phone format: +41 44 499 00 75 or 044 499 00 75 (supports 2-digit
            // groups)
            Pattern.compile("\\+41[-.\\s]?\\d{2}[-.\\s]?\\d{3}[-.\\s]?\\d{2}[-.\\s]?\\d{2}", Pattern.CASE_INSENSITIVE), // Swiss
                                                                                                                        // international
                                                                                                                        // format
                                                                                                                        // (+41)
            Pattern.compile("0\\d{2}[-.\\s]?\\d{3}[-.\\s]?\\d{2}[-.\\s]?\\d{2}", Pattern.CASE_INSENSITIVE), // Swiss
                                                                                                            // landline
                                                                                                            // format
                                                                                                            // (starts
                                                                                                            // with 0)
            // More flexible international format supporting 2-4 digit groups
            Pattern.compile("\\+[1-9]\\d{1,3}[-.\\s]?\\d{2,4}(?:[-.\\s]?\\d{2,4}){2,3}", Pattern.CASE_INSENSITIVE), // Flexible
                                                                                                                    // international
                                                                                                                    // format

            // Address patterns (street addresses, postal codes, city/state/country
            // combinations)
            Pattern.compile(
                    "\\d+\\s+[a-zA-Z0-9\\s,.-]+\\s+(?:street|st|avenue|ave|road|rd|boulevard|blvd|lane|ln|drive|dr|court|ct|place|pl|way|blvd)\\b",
            ...
            Pattern.compile("\\b\\d{4}\\s+[A-ZÄÖÜ][a-zäöüß]+(?:\\s+[A-ZÄÖÜ][a-zäöüß]+)*\\b", Pattern.CASE_INSENSITIVE), // Swiss
                                                                                                                        // postal
                                                                                                                        // codes
            Pattern.compile("\\b[a-zA-Z]+\\s+(?:road|rd|street|st|avenue|ave|boulevard|blvd|lane|ln|drive|dr)\\b",
                    Pattern.CASE_INSENSITIVE), // Road names without numbers (e.g., "Spintex Road")
            Pattern.compile(
                    ...
                    Pattern.CASE_INSENSITIVE), // City/State/Country combinations (e.g., "Winnipeg Manitoba Canada",
                                               // "Accra Greater Accra Region Ghana", "Zürich Schweiz")
            Pattern.compile(
                    "\\b(?:Canada|USA|United States|Ghana|UK|United Kingdom|Australia|New Zealand|South Africa|India|Brazil|Mexico|Germany|France|Italy|Spain|Netherlands|Belgium|Switzerland|Schweiz|Suisse|Svizzera|Austria|Sweden|Norway|Denmark|Finland|Poland|Portugal|Greece|Ireland|Japan|China|South Korea|Singapore|Malaysia|Thailand|Philippines|Indonesia|Vietnam|Taiwan|Hong Kong|UAE|Saudi Arabia|Israel|Turkey|Egypt|Kenya|Nigeria|Argentina|Chile|Colombia|Peru|Venezuela|Ecuador|Uruguay|Paraguay|Bolivia)\\b",
                    Pattern.CASE_INSENSITIVE), // Country names alone (e.g., "Canada and US", "Schweiz")
    ...
    public SpamDetectionResult detectSpam(SpamDetectionContext context) {
        ...
        try {
            ...
            final boolean marketingHeavy = contentExtractor
                    .countSpamKeywords(content.toLowerCase()) >= MARKETING_KEYWORD_THRESHOLD;

            // Count nodes from the parsed model
            int topicCount = mapModel.getTotalTopicCount();

            // Extract and normalize content early to check for contact info before
            // exemption
            ...
            // Phone patterns: indices 2-10 (9 patterns including Swiss and flexible
            // international)
            ...
            // If map has complete contact info (address + phone + website/email), it's
            // likely spam
            // even with higher node counts, unless it's a very large legitimate map (> 30
            // topics)
            boolean hasCompleteContactInfo = hasAddress && hasPhone && (hasWebsite || hasEmail);

            // Exempt high node count maps unless they're marketing-heavy OR have complete
            // contact info
            // Complete contact info is a strong spam signal, so we check it even for higher
            // node counts
            ...
            // Rule 1: Complete contact info combo (address + phone + website/email) with
            // low node count
            ...
            if ((hasPhone || hasWebsite || hasEmail) &&
                ...
                return SpamDetectionResult.spam(
                        ...
                        String.format(
                                "ContactPatterns: %d, Separators: %d, NearMeRepetitions: %d, LocationVariants: %d",
            ...
            if (contactPatternMatches >= 4 && marketingHeavy && topicCount <= 5) {
                return SpamDetectionResult.spam(
                        ...
                        String.format("ContactPatterns: %d, MarketingKeywords: %d, TopicCount: %d",
                                contactPatternMatches, contentExtractor.countSpamKeywords(content.toLowerCase()),
                                topicCount),
                        getType());
            }

            // Rule 5: Address + Website/Email + multiple locations (even without phone)
            // This catches spam maps that have address and contact info but omit phone
            // number
            if (hasAddress && (hasWebsite || hasEmail) && locationVariants >= 5 && topicCount <= 5) {
                return SpamDetectionResult.spam(
                        ...
                        String.format(
                                "HasAddress: %s, HasWebsite: %s, HasEmail: %s, LocationVariants: %d, TopicCount: %d",
                                hasAddress, hasWebsite, hasEmail, locationVariants, topicCount),
                        getType());
            }

            // Rule 6: Address + Website + low node count (catches business listings even
            // without phone)
            // This catches spam maps like 1901495 and 1924052 that have address + website
            // but no phone
            ...
            // Rule 7: Complete contact info (address + phone + website/email) with moderate
            // node count
            // This catches spam maps like 1922448 that have complete contact info but more
            // than 5 topics
            // We allow up to 10 topics to catch service directory spam that includes some
            // additional content
            ...
            // Rule 9: Complete contact info with higher node count (catches spam maps with
            // tutorial content)
            // This catches spam maps like 1764490 that have complete contact info but
            // include tutorial content
            // We allow up to 30 topics to catch spam that mixes contact info with other
            // content
            ...
            // Rule 8: Address + Website/Email + moderate node count (catches business
            // listings without phone)
            // This catches spam maps that have address and contact info but no phone, with
            // more than 3 topics
